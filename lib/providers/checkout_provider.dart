...
import 'package:eden/repositories/address_repository.dart';
import 'package:eden/repositories/shipping_repository.dart';
import 'package:eden/repositories/cart_repository.dart';
import 'package:eden/repositories/payment_repository.dart';
import 'package:eden/repositories/business_setting_repository.dart';
...
class CheckoutProvider extends ChangeNotifier {
  void _logSilentError(Object error) {
  ...
  void clearNameError({bool isBilling = false}) {
    if (isBilling) {
      if (billingNameErrorText != null) {
        billingNameErrorText = null;
        notifyListeners();
      }
    } else {
  ...
  void clearAddressError({bool isBilling = false}) {
    if (isBilling) {
      if (billingAddressErrorText != null) {
        billingAddressErrorText = null;
        notifyListeners();
      }
    } else {
  ...
  void clearPhoneError({bool isBilling = false}) {
    if (isBilling) {
      if (billingPhoneErrorText != null) {
        billingPhoneErrorText = null;
        notifyListeners();
      }
    } else {
  ...
  Future<bool> submitGuestAddress() async {
    bool isValid = true;
  ...
  Future<void> confirmOrder(BuildContext context) async {
    ...
    if (guest_checkout_status.$ && !is_logged_in.$) {
      bool success = await submitGuestAddress();
      if (!success) return;
    }
